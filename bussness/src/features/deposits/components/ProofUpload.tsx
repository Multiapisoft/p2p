...
export function ProofUpload({
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const uploaded = await businessDepositPayApi.uploadProof(file);
      ...
      {preview && (
        <div className="overflow-hidden rounded-xl border border-outline-variant">
          ...
          <img
            src={preview}
            alt="Payment proof"
            className="max-h-56 w-full bg-black/5 object-contain"
          />
        </div>
      )}

      <Input
        label={isTxid ? `TxID${utrRequired ? ' *' : ''}` : `UTR${utrRequired ? ' *' : ''}`}
        value={utr}
        onChange={(e) => onUtrChange(e.target.value)}
        placeholder={isTxid ? 'Transaction hash' : '12-digit UTR'}
        disabled={disabled}
