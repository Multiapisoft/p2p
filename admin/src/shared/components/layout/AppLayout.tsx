...
export function AppLayout({ children }: { children: ReactNode }) {
  ...
  const visibleNav = NAV_ITEMS.filter((item) => has(item.permission));
  ...
  return (
    <div className="flex h-dvh overflow-hidden bg-background">
      {/* Desktop sidebar — fixed height; only nav list scrolls */}
      <aside className="hidden h-dvh w-64 shrink-0 flex-col border-r border-outline-variant bg-surface md:flex">
        ...
        <nav className="custom-scrollbar min-h-0 flex-1 space-y-1 overflow-y-auto overscroll-contain p-4">
          {visibleNav.map((item) => {
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        <header className="z-40 flex shrink-0 items-center justify-between border-b border-outline-variant bg-surface/95 px-3 py-2.5 backdrop-blur-sm sm:px-4 sm:py-3">
        ...
        <main className="custom-scrollbar min-h-0 flex-1 overflow-x-hidden overflow-y-auto overscroll-contain px-3 py-4 pb-[calc(4.5rem+env(safe-area-inset-bottom))] sm:px-4 sm:py-5 md:px-8 md:pb-8 md:py-6">
          {children}
        </main>
        ...
        >
          <div className="flex justify-around">
            {mobilePrimary.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.to}
                  className={cn(
                    'flex min-w-0 flex-1 flex-col items-center rounded-lg px-0.5 py-1 text-[9px] transition-transform active:scale-95 sm:text-[10px]',
            ...
            <button
              ...
              className={cn(
                'flex min-w-0 flex-1 flex-col items-center rounded-lg px-0.5 py-1 text-[9px] sm:text-[10px]',
          ...
          {mobileMenuOpen && (
            <div className="custom-scrollbar mt-1.5 max-h-[40vh] overflow-y-auto border-t border-outline-variant pt-1.5">
