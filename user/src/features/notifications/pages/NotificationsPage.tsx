    ...
    <div className="mx-auto max-w-4xl space-y-3 sm:space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <div className="rounded-lg border border-outline-variant bg-surface-container-lowest p-2 sm:p-2.5">
          ...
          <p className="mt-0.5 text-base font-bold sm:text-lg">{unreadOnPage}</p>
            ...
            <div className={`space-y-2 ${isFetching ? 'opacity-70' : ''}`}>
              {items.map((n) => (
                <button
                  ...
                  className={`w-full rounded-lg border border-outline-variant px-2.5 py-2 text-left transition-colors hover:bg-surface-container-low ${
                    !n.isRead ? 'bg-secondary-container/20' : ''
                  }`}
                >
                  <p className="text-sm font-semibold">{n.title}</p>
                  <p className="mt-0.5 line-clamp-2 text-xs text-on-surface-variant">{n.message}</p>
                  <p className="mt-1 text-[11px] text-outline">{formatDate(n.createdAt)}</p>
                </button>
              ))}
            </div>
            <div className="mt-3">
