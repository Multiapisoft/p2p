        ...
        <>
          ...
          <div className="absolute right-0 z-50 mt-2 w-[min(20rem,calc(100vw-1.5rem))] overflow-hidden rounded-xl border border-outline-variant bg-surface shadow-lg">
              ...
              {!data?.items.length ? (
                ...
                data.items.slice(0, 8).map((n) => (
                  <button
                    ...
                    className={`w-full border-b border-outline-variant px-3 py-2.5 text-left sm:px-4 sm:py-3 ${!n.isRead ? 'bg-secondary-container/20' : ''}`}
                  >
            ...
            <Link
              ...
              className="block border-t border-outline-variant px-3 py-2.5 text-center text-xs font-semibold text-secondary hover:bg-surface-container-low sm:px-4"
            >
              View all
            </Link>
          </div>
