...
import { fulfillApi } from '@/features/fulfill/api/fulfill.api';
...
export function ProofUpload({
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const uploaded = await fulfillApi.uploadProof(file, 'withdrawal-payment-proof');
    ...
    <div className="space-y-3">
      <div>
        ...
        <p className="mt-1 text-xs text-on-surface-variant">
          {isTxid
            ? 'Upload transfer proof, then enter the TxID / TRX hash below. Same TxID cannot be reused.'
            : 'Your payment proof will be uploaded securely. Enter the UTR below. Same UTR cannot be reused.'}
        </p>
      </div>
      ...
      <Input
        label={isTxid ? 'TxID / TRX hash *' : 'UTR / Transaction Reference *'}
        value={utr}
        onChange={(e) =>
          onUtrChange(isTxid ? e.target.value.trim() : e.target.value.toUpperCase())
        }
        placeholder={
          isTxid ? 'Blockchain transaction hash / TxID' : '12-digit UTR or reference number'
        }
        required
        disabled={disabled}
      />
